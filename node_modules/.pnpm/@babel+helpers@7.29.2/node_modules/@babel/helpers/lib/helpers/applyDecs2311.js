...
function applyDecs2311(targetClass, classDecs, memberDecs, classDecsHaveThis, instanceBrand, parentClass) {
  ...
  function applyDec(Class, decInfo, decoratorsHaveThis, name, kind, initializers, ret, isStatic, isPrivate, isField, hasPrivateBrand) {
    ...
    if (!isClass) {
      ...
      if (isPrivate) {
        if (isField || isAccessor) {
          desc = {
            get: (0, _setFunctionName.default)(function  get() {
              return decVal(this);
            }, name, "get"),
    ...
    for (var i = decs.length - 1; i >= 0; i -= decoratorsHaveThis ? 2 : 1) {
      ...
      var ctx = {
        ...
        name: name,
        metadata: metadata,
        addInitializer: function  addInitializer(decoratorFinishedRef, initializer) {
      ...
      if (isClass) {
        ...
        _ = ctx.access = {
          has: isPrivate ? hasPrivateBrand.bind() : function  has(target) {
            return name in target;
          }
        };
        if (!isSetter) {
          _.get = isPrivate ? isMethod ? function  get(_this) {
            assertInstanceIfPrivate(_this);
            return desc.value;
          } : _bindPropCall("get", 0, assertInstanceIfPrivate) : function  get(target) {
            return target[name];
          };
        }
        if (!isMethod && !isGetter) {
          _.set = isPrivate ? _bindPropCall("set", 0, assertInstanceIfPrivate) : function  set(target, v) {
            target[name] = v;
          };
        }
  ...
  function applyMemberDecs() {
    ...
    var pushInitializers = function  pushInitializers(initializers) {
      if (initializers) {
        ret.push(createRunInitializers(initializers));
      }
    };
    var applyMemberDecsOfKind = function  applyMemberDecsOfKind(isStatic, isField) {
      for (var i = 0; i < memberDecs.length; i++) {
        ...
        if ((kind & 8) == isStatic && !kindOnly == isField) {
          ...
          applyDec(isStatic ? targetClass : targetClass.prototype, decInfo, decoratorsHaveThis, isPrivate ? "#" + name : (0, _toPropertyKey.default)(name), kindOnly, kindOnly < 2 ? [] : isStatic ? staticInitializers = staticInitializers || [] : protoInitializers = protoInitializers || [], ret, !!isStatic, isPrivate, isField, isStatic && isPrivate ? function  applyMemberDecsOfKind(_) {
