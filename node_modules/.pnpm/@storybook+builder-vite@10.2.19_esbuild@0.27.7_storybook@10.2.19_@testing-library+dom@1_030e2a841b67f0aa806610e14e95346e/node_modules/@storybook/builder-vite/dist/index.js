...
async function transformIframeHtml(html, options) {
  ...
  })), otherGlobals = build3?.test?.disableBlocks ? { __STORYBOOK_BLOCKS_EMPTY_MODULE__: {} } : {}, transformedHtml = html.replace("[CONFIG_TYPE HERE]", configType || "").replace("[LOGLEVEL HERE]", logLevel || "").replace("'[FRAMEWORK_OPTIONS HERE]'", JSON.stringify(frameworkOptions)).replace(
    "('OTHER_GLOBALS HERE');",
...
function pluginWebpackStats({ workingDir }) {
  ...
  return {
    ...
    enforce: "post",
    moduleParsed: function moduleParsed(mod) {
