...
var require_parser = __commonJS({
  "../../../node_modules/react-docgen-typescript/lib/parser.js"(exports) {
    ...
    }, __spreadArray = exports && exports.__spreadArray || function __spreadArray(to, from) {
    ...
    function withCustomConfig(tsconfigPath, parserOpts) {
      var basePath = path.dirname(tsconfigPath), _a = ts.readConfigFile(tsconfigPath, function _a(filename) {
    ...
    function withCompilerOptions(compilerOptions, parserOpts) {
      return parserOpts === void 0 && (parserOpts = exports.defaultParserOpts), {
        parse: function parse(filePathOrPaths) {
    ...
    var isOptional = function isOptional(prop) {
      ...
      (function Parser() {
        ...
        return Parser2.prototype.getTypeSymbol = function getTypeSymbol(exp) {
          ...
          return typeSymbol;
        }, Parser2.prototype.isPlainObjectType = function isPlainObjectType(exp) {
          ...
          return isPlain;
        }, Parser2.prototype.getComponentExports = function getComponentExports(exp) {
          var targetSymbol = exp;
          ...
          if (exp.flags & ts.SymbolFlags.Alias && (targetSymbol = this.checker.getAliasedSymbol(exp)), targetSymbol.exports)
            return { symbol: targetSymbol, exports: targetSymbol.exports };
        }, Parser2.prototype.getComponentFromExpression = function getComponentFromExpression(exp) {
          ...
          return exp;
        }, Parser2.prototype.getComponentInfo = function getComponentInfo(exp, source, componentNameResolver, customComponentTypes) {
          if (componentNameResolver === void 0 && (componentNameResolver = function componentNameResolver() {
        ...
        }, Parser2.prototype.extractPropsFromTypeIfStatelessComponent = function extractPropsFromTypeIfStatelessComponent(type) {
          ...
          return null;
        }, Parser2.prototype.extractPropsFromTypeIfStatefulComponent = function extractPropsFromTypeIfStatefulComponent(type) {
          ...
          return null;
        }, Parser2.prototype.extractMembersFromType = function extractMembersFromType(type) {
          var _this = this, methodSymbols = [];
          return type.getProperties().forEach(function extractMembersFromType(property) {
            _this.getCallSignature(property) && methodSymbols.push(property);
          }), type.symbol && type.symbol.members && type.symbol.members.forEach(function extractMembersFromType(member) {
            methodSymbols.push(member);
          }), methodSymbols;
        }, Parser2.prototype.getMethodsInfo = function getMethodsInfo(type) {
          var _this = this, members = this.extractMembersFromType(type), methods = [];
          return members.forEach(function getMethodsInfo(member) {
        ...
        }, Parser2.prototype.getModifiers = function getModifiers(member) {
          var modifiers = [];
        ...
        }, Parser2.prototype.getParameterInfo = function getParameterInfo(callSignature) {
          var _this = this;
          return callSignature.parameters.map(function getParameterInfo(param) {
        ...
        }, Parser2.prototype.getCallSignature = function getCallSignature(symbol) {
        ...
        }, Parser2.prototype.isTaggedPublic = function isTaggedPublic(symbol) {
          var jsDocTags = symbol.getJsDocTags();
          return !!jsDocTags.find(function isTaggedPublic(tag) {
            return tag.name === "public";
          });
        }, Parser2.prototype.getReturnDescription = function getReturnDescription(symbol) {
          var tags = symbol.getJsDocTags(), returnTag = tags.find(function returnTag(tag) {
            return tag.name === "returns";
          });
          if (!(!returnTag || !Array.isArray(returnTag.text)))
            return returnTag.text;
        }, Parser2.prototype.getValuesFromUnionType = function getValuesFromUnionType(type) {
          return type.isStringLiteral() ? '"' + type.value + '"' : type.isNumberLiteral() ? "" + type.value : this.checker.typeToString(type);
        }, Parser2.prototype.getInfoFromUnionType = function getInfoFromUnionType(type) {
          var commentInfo = {};
        ...
        }, Parser2.prototype.getDocgenType = function getDocgenType(propType, isRequired) {
          var _this = this;
          propType.getConstraint() && (propType = propType.getConstraint());
          var propTypeString = this.checker.typeToString(propType);
          if (this.shouldRemoveUndefinedFromOptional && !isRequired && (propTypeString = propTypeString.replace(" | undefined", "")), propType.isUnion() && (this.shouldExtractValuesFromUnion || this.shouldExtractLiteralValuesFromEnum && propType.types.every(function getDocgenType(type) {
            ...
            var value = propType.types.map(function value(type) {
              return _this.getInfoFromUnionType(type);
            });
            return this.shouldRemoveUndefinedFromOptional && !isRequired && (value = value.filter(function value(option) {
              return option.value != "undefined";
            })), this.shouldSortUnions && value.sort(function getDocgenType(a, b) {
        ...
        }, Parser2.prototype.getPropsInfo = function getPropsInfo(propsObj, defaultProps) {
          var _this = this;
          ...
          var result = {};
          return propertiesOfProps.forEach(function getPropsInfo(prop) {
            ...
            if (_this.propertiesOfPropsCache.has(cacheKey))
              ...
              var parents = getDeclarations(prop), declarations = prop.declarations || [], baseProp = baseProps.find(function baseProp(p) {
              ...
              // @ts-ignore
              declarations.every(function required(d) {
...
`), propItem = __assign({
        ...
        }, Parser2.prototype.findDocComment = function findDocComment(symbol) {
          var _this = this, comment = this.getFullJsDocComment(symbol);
          if (comment.fullComment || comment.tags.default)
            return comment;
          var rootSymbols = this.checker.getRootSymbols(symbol), commentsOnRootSymbols = rootSymbols.filter(function commentsOnRootSymbols(x) {
            return x !== symbol;
          }).map(function commentsOnRootSymbols(x) {
            return _this.getFullJsDocComment(x);
          }).filter(function commentsOnRootSymbols(x) {
            return !!x.fullComment || !!comment.tags.default;
          });
          return commentsOnRootSymbols.length ? commentsOnRootSymbols[0] : defaultJSDoc;
        }, Parser2.prototype.getFullJsDocComment = function getFullJsDocComment(symbol) {
...
`));
          ...
          return tags.forEach(function getFullJsDocComment(tag) {
...
`)).trim(),
        ...
        }, Parser2.prototype.getFunctionStatement = function getFunctionStatement(statement) {
        ...
        }, Parser2.prototype.extractDefaultPropsFromComponent = function extractDefaultPropsFromComponent(symbol, source) {
          var _this = this, possibleStatements = __spreadArray(__spreadArray([], source.statements.filter(function possibleStatements(stmt) {
            return !!stmt.name;
          }).filter(function possibleStatements(stmt) {
            return _this.checker.getSymbolAtLocation(stmt.name) === symbol;
          })), source.statements.filter(function possibleStatements(stmt) {
            return ts.isExpressionStatement(stmt) || ts.isVariableStatement(stmt);
          }));
          return possibleStatements.reduce(function extractDefaultPropsFromComponent(res, statement) {
            if (statementIsClassDeclaration(statement) && statement.members.length) {
              var possibleDefaultProps = statement.members.filter(function possibleDefaultProps(member) {
              ...
              var propMap_1 = {};
              return statement.getChildren().forEach(function extractDefaultPropsFromComponent(child) {
        ...
        }, Parser2.prototype.getLiteralValueFromImportSpecifier = function getLiteralValueFromImportSpecifier(property) {
          ...
          return null;
        }, Parser2.prototype.getLiteralValueFromPropertyAssignment = function getLiteralValueFromPropertyAssignment(property) {
        ...
        }, Parser2.prototype.getPropMap = function getPropMap(properties) {
          var _this = this;
          return properties.reduce(function getPropMap(acc, property) {
    ...
    function getTextValueOfClassMember(classDeclaration, memberName) {
      var classDeclarationMembers = classDeclaration.members || [], textValue = (classDeclarationMembers && classDeclarationMembers.filter(function textValue(member) {
        return ts.isPropertyDeclaration(member);
      }).filter(function textValue(member) {
        var name = ts.getNameOfDeclaration(member);
        return name && name.text === memberName;
      }).map(function textValue(member) {
        var property = member;
    ...
    function getTextValueOfFunctionProperty(exp, source, propertyName) {
      var identifierStatements = source.statements.filter(function identifierStatements(statement) {
        return ts.isExpressionStatement(statement);
      }).filter(function identifierStatements(statement) {
        var _a, _b, _c, _d, expr = statement.expression, locals = Array.from(source.locals), hasOneLocalExport = locals.filter(function hasOneLocalExport(local) {
      ...
      }).map(function identifierStatements(statement) {
      ...
      if (identifierStatements.length > 0) {
        var locatedStatement = identifierStatements.find(function locatedStatement(statement) {
    ...
    function parseWithProgramProvider(filePathOrPaths, compilerOptions, parserOpts, programProvider) {
      ...
      return filePaths.map(function(filePath) {
        ...
        var componentDocsNoDuplicates = componentDocs.reduce(function componentDocsNoDuplicates(prevVal, comp) {
          var duplicate = prevVal.find(function duplicate(compDoc) {
            return compDoc.displayName === comp.displayName;
          });
          return duplicate ? prevVal : __spreadArray(__spreadArray([], prevVal), [comp]);
        }, []), filteredComponentDocs = componentDocsNoDuplicates.filter(function filteredComponentDocs(comp, index, comps) {
          return comps.slice(index + 1).every(function filteredComponentDocs(innerComp) {
...
var require_lib = __commonJS({
  "../../../node_modules/react-docgen-typescript/lib/index.js"(exports) {
    ...
    Object.defineProperty(exports, "getDefaultExportForFile", { enumerable: !0, get: function get() {
      return parser_1.getDefaultExportForFile;
    } });
    Object.defineProperty(exports, "parse", { enumerable: !0, get: function get() {
      return parser_1.parse;
    } });
    Object.defineProperty(exports, "Parser", { enumerable: !0, get: function get() {
      return parser_1.Parser;
    } });
    Object.defineProperty(exports, "withCompilerOptions", { enumerable: !0, get: function get() {
      return parser_1.withCompilerOptions;
    } });
    Object.defineProperty(exports, "withCustomConfig", { enumerable: !0, get: function get() {
      return parser_1.withCustomConfig;
    } });
    Object.defineProperty(exports, "withDefaultConfig", { enumerable: !0, get: function get() {
