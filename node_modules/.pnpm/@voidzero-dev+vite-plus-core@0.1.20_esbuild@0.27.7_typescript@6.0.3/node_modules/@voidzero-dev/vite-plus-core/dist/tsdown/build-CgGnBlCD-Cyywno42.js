...
const resolveSymlinksAsync = function resolveSymlinksAsync(path, state, callback$1) {
...
const resolveSymlinks = function resolveSymlinks(path, state, callback$1) {
	const { queue, fs, options: { suppressErrors } } = state;
	queue.enqueue();
	try {
...
const isFile = quansync(function*  isFile(path, allowSymlinks) {
	try {
...
const findUp = quansync(function*  findUp(paths, options = {}) {
...
const loadConfigFile$1 = quansync(function*  loadConfigFile$1(filepath, source) {
	try {
...
function createConfigCoreLoader(options) {
	...
	const results = [];
	let matchedFiles;
	const findConfigs = quansync(function*  findConfigs() {
	...
	return {
		load: quansync(function*  load(force = false) {
...
function NodeProtocolPlugin(nodeProtocolOption) {
	...
	return {
		...
		resolveId: {
			...
			handler: nodeProtocolOption === "strip" ? async function handler(id, ...args) {
