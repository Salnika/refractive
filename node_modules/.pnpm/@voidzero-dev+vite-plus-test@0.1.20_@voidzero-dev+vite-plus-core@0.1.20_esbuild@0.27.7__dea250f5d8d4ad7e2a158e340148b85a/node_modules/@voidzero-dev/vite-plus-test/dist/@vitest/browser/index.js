...
var version = process.env.VP_VERSION || "4.1.5";
...
const _EXTNAME_RE = /.(\.[^./]+|\.)$/;
const normalize = function normalize(path) {
  if (path.length === 0) {
    return ".";
  }
...
const join = function join(...segments) {
  let path = "";
...
const extname = function extname(p) {
  if (p === "..") return "";
...
const dirname = function dirname(p) {
...
const basename = function basename(p, extension) {
...
var __classPrivateFieldGet = (null && null.__classPrivateFieldGet) || function  __classPrivateFieldGet(receiver, state, kind, f) {
...
const between = function between(num, first, last) {
...
function tokenize(str1) {
  ...
  const tokens = [];
  let code;
  const codepoint = function codepoint(i) {
    if (i >= str.length) return -1;
    return str[i];
  };
  const next = function next(num) {
    if (num === void 0) num = 1;
    if (num > 3) throw "Spec Error: no more than three codepoints of lookahead.";
    return codepoint(i + num);
  };
  const consume = function consume(num) {
    if (num === void 0) num = 1;
    i += num;
    code = codepoint(i);
    return true;
  };
  const reconsume = function reconsume() {
    i -= 1;
    return true;
  };
  const eof = function eof(codepoint) {
    if (codepoint === void 0) codepoint = code;
    return codepoint === -1;
  };
  const consumeAToken = function consumeAToken() {
    consumeComments();
    consume();
    if (whitespace(code)) {
  ...
  const consumeComments = function consumeComments() {
  ...
  const consumeANumericToken = function consumeANumericToken() {
  ...
  const consumeAnIdentlikeToken = function consumeAnIdentlikeToken() {
  ...
  const consumeAStringToken = function consumeAStringToken(endingCodePoint) {
  ...
  const consumeAURLToken = function consumeAURLToken() {
  ...
  const consumeEscape = function consumeEscape() {
    consume();
    if (hexdigit(code)) {
      const digits = [code];
  ...
  const areAValidEscape = function areAValidEscape(c1, c2) {
    if (c1 !== 92) return false;
    if (newline(c2)) return false;
    return true;
  };
  const startsWithAValidEscape = function startsWithAValidEscape() {
    return areAValidEscape(code, next());
  };
  const wouldStartAnIdentifier = function wouldStartAnIdentifier(c1, c2, c3) {
  ...
  const startsWithAnIdentifier = function startsWithAnIdentifier() {
    return wouldStartAnIdentifier(code, next(1), next(2));
  };
  const wouldStartANumber = function wouldStartANumber(c1, c2, c3) {
    if (c1 === 43 || c1 === 45) {
  ...
  const startsWithANumber = function startsWithANumber() {
    return wouldStartANumber(code, next(1), next(2));
  };
  const consumeAName = function consumeAName() {
    let result = "";
  ...
  const consumeANumber = function consumeANumber() {
    let repr = "";
    let type = "integer";
    if (next() === 43 || next() === 45) {
      consume();
  ...
  const convertAStringToANumber = function convertAStringToANumber(string) {
    return +string;
  };
  const consumeTheRemnantsOfABadURL = function consumeTheRemnantsOfABadURL() {
