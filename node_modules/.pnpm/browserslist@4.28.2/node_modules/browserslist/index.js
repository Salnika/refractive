...
function filterByYear(since, context) {
  ...
  return Object.keys(agents).reduce(function (selected, name) {
    ...
    var versions = Object.keys(data.releaseDate).filter(function  versions(v) {
...
function resolve(queries, context) {
  return parseQueries(queries).reduce(function (result, node, index) {
    ...
    var array = type.select.call(browserslist, context, node).map(function  array(j) {
      var parts = j.split(' ')
      if (parts[1] === '0') {
...
function browserslist(queries, opts) {
  ...
  var needsPath = parseQueries(queries).some(function  needsPath(node) {
  ...
  var result = uniq(resolve(queries, context)).sort(function  result(name1, name2) {
    name1 = name1.split(' ')
    name2 = name2.split(' ')
    if (name1[0] === name2[0]) {
...
browserslist.parse = function  parse(queries, opts) {
  opts = prepareOpts(opts)
  queries = prepareQueries(queries, opts)
  checkQueries(queries)
...
browserslist.coverage = function  coverage(browsers, stats) {
  var data
  ...
  return browsers.reduce(function  coverage(all, i) {
    var usage = data[i]
    if (usage === undefined) {
...
function nodeQuery(context, node) {
  var matched = browserslist.nodeVersions.filter(function  matched(i) {
...
function coverQuery(context, node) {
  ...
  var versions = Object.keys(usage).sort(function  versions(a, b) {
...
var QUERIES = {
  last_major_versions: {
    ...
    select: function  select(context, node) {
      return Object.keys(agents).reduce(function  select(selected, name) {
        var data = byName(name, context)
        if (!data) return selected
  ...
  last_electron_major_versions: {
    ...
    select: function  select(context, node) {
      ...
      return validVersions.map(function  select(i) {
        return 'chrome ' + e2c[i]
      })
    }
  },
  ...
  unreleased_versions: {
    ...
    select: function  select(context) {
      return Object.keys(agents).reduce(function  select(selected, name) {
        var data = byName(name, context)
        if (!data) return selected
        var list = data.versions.filter(function  list(v) {
  ...
  popularity: {
    ...
    select: function  select(context, node) {
      ...
      var usage = browserslist.usage.global
      return Object.keys(usage).reduce(function  select(result, version) {
        if (node.sign === '>') {
  ...
  supports: {
    ...
    select: function  select(context, node) {
      ...
      for (var name in features) {
        ...
        data.versions.forEach(function  select(version) {
...
;(function () {
  ...
  browserslist.nodeVersions = jsReleases.map(function  nodeVersions(release) {
