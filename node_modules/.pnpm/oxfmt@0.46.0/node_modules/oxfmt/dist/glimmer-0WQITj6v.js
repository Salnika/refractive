...
var Mn = String.prototype.replaceAll ?? function Mn(e, t) {
	return e.global ? this.replace(e, t) : this.split(e).join(t);
}, St = Jt("replaceAll", function St() {
	if (typeof this === "string") return Mn;
});
function Yn(e) {
	return this[e < 0 ? this.length + e : e];
}
var V = Jt("at", function V() {
...
var Vt = (function Vt() {
	var e = function e(X, b, S, y) {
		...
		performAction: function performAction(b, S, y, m, w, l, bt) {
			var u = l.length - 1;
			switch (w) {
				case 1: return l[u - 1];
				case 2:
		...
		parseError: function parseError(b, S) {
...
Expecting ` + Qt.join(", ") + ", got '" + (this.terminals_[L] || L) + "'" : De = "Parse error on line " + (Z + 1) + ": Unexpected " + (L == Pr ? "end of input" : "'" + (this.terminals_[L] || L) + "'"), this.parseError(De, {
	...
	Ne.lexer = (function lexer() {
		return {
			EOF: 1,
			parseError: function parseError(S, y) {
				if (this.yy.parser) this.yy.parser.parseError(S, y);
				else throw new Error(S);
			},
			setInput: function setInput(b, S) {
...
` + S + "^";
			...
			test_match: function test_match(b, S) {
				var y, m, w;
...
` + this.showPosition(), {
			...
			options: {},
			performAction: function performAction(S, y, m, w) {
				function l(u, Z) {
...
le.prototype = {
	constructor: le,
	mutating: !1,
	acceptKey: function acceptKey(e, t) {
	...
	acceptRequired: function acceptRequired(e, t) {
		if (this.acceptKey(e, t), !e[t]) throw new tt(e.type + " requires " + t);
	},
	acceptArray: function acceptArray(e) {
	...
	DecoratorBlock: Gr,
	PartialStatement: Kr,
	PartialBlockStatement: function PartialBlockStatement(e) {
...
G.prototype = new Wr();
G.prototype.Program = function Program(e) {
...
G.prototype.BlockStatement = G.prototype.DecoratorBlock = G.prototype.PartialBlockStatement = function PartialBlockStatement(e) {
...
G.prototype.Decorator = G.prototype.MustacheStatement = function MustacheStatement(e) {
	return e.strip;
};
G.prototype.PartialStatement = G.prototype.CommentStatement = function CommentStatement(e) {
	var t = e.strip || {};
	return {
		inlineStandalone: !0,
		open: t.open,
...
function he(e, t) {
	...
	Vt.yy = Qr, Vt.yy.locInfo = function locInfo(o) {
...
`,
	...
	zwj: "‍",
	zwnj: "‌"
}, ws = /^#[xX]([A-Fa-f0-9]+)$/, Ts = /^#([0-9]+)$/, xs = /^([A-Za-z0-9]+)$/, $e = (function $e() {
	function e(t) {
		this.named = t;
	}
	return e.prototype.parse = function parse(t) {
		if (t) {
			var r = t.match(ws);
...
`) {
	...
	return e.prototype.reset = function reset() {
		this.transitionTo("beforeData"), this.input = "", this.tagNameBuffer = "", this.index = 0, this.line = 1, this.column = 0, this.delegate.reset();
	}, e.prototype.transitionTo = function transitionTo(t) {
		this.state = t;
	}, e.prototype.tokenize = function tokenize(t) {
		this.reset(), this.tokenizePart(t), this.tokenizeEOF();
	}, e.prototype.tokenizePart = function tokenizePart(t) {
	...
	}, e.prototype.tokenizeEOF = function tokenizeEOF() {
		this.flushData();
	}, e.prototype.flushData = function flushData() {
		this.state === "data" && (this.delegate.finishData(), this.transitionTo("beforeData"));
	}, e.prototype.peek = function peek() {
		return this.input.charAt(this.index);
	}, e.prototype.consume = function consume() {
		var t = this.peek();
		return this.index++, t === `
` ? (this.line++, this.column = 0) : this.column++, t;
	}, e.prototype.consumeCharRef = function consumeCharRef() {
	...
	}, e.prototype.markTagStart = function markTagStart() {
		this.delegate.tagOpen();
	}, e.prototype.appendToTagName = function appendToTagName(t) {
		this.tagNameBuffer += t, this.delegate.appendToTagName(t);
	}, e.prototype.isIgnoredEndTag = function isIgnoredEndTag() {
...
(function() {
	...
	return e.prototype.tokenize = function tokenize(t) {
	...
	}, e.prototype.reset = function reset() {
		this.token = null, this.startLine = 1, this.startColumn = 0;
	}, e.prototype.current = function current() {
		var t = this.token;
	...
	}, e.prototype.push = function push(t) {
		this.token = t, this.tokens.push(t);
	}, e.prototype.currentAttribute = function currentAttribute() {
		return this._currentAttribute;
	}, e.prototype.addLocInfo = function addLocInfo() {
	...
	}, e.prototype.beginDoctype = function beginDoctype() {
		this.push({
			type: "Doctype",
			name: ""
		});
	}, e.prototype.appendToDoctypeName = function appendToDoctypeName(t) {
		this.current("Doctype").name += t;
	}, e.prototype.appendToDoctypePublicIdentifier = function appendToDoctypePublicIdentifier(t) {
		var r = this.current("Doctype");
		r.publicIdentifier === void 0 ? r.publicIdentifier = t : r.publicIdentifier += t;
	}, e.prototype.appendToDoctypeSystemIdentifier = function appendToDoctypeSystemIdentifier(t) {
		var r = this.current("Doctype");
		r.systemIdentifier === void 0 ? r.systemIdentifier = t : r.systemIdentifier += t;
	}, e.prototype.endDoctype = function endDoctype() {
		this.addLocInfo();
	}, e.prototype.beginData = function beginData() {
		this.push({
			type: "Chars",
			chars: ""
		});
	}, e.prototype.appendToData = function appendToData(t) {
		this.current("Chars").chars += t;
	}, e.prototype.finishData = function finishData() {
		this.addLocInfo();
	}, e.prototype.beginComment = function beginComment() {
		this.push({
			type: "Comment",
			chars: ""
		});
	}, e.prototype.appendToCommentData = function appendToCommentData(t) {
		this.current("Comment").chars += t;
	}, e.prototype.finishComment = function finishComment() {
		this.addLocInfo();
	}, e.prototype.tagOpen = function tagOpen() {}, e.prototype.beginStartTag = function beginStartTag() {
		this.push({
			type: "StartTag",
			tagName: "",
			attributes: [],
	...
	}, e.prototype.beginEndTag = function beginEndTag() {
		this.push({
			type: "EndTag",
			tagName: ""
		});
	}, e.prototype.finishTag = function finishTag() {
		this.addLocInfo();
	}, e.prototype.markTagAsSelfClosing = function markTagAsSelfClosing() {
		this.current("StartTag").selfClosing = !0;
	}, e.prototype.appendToTagName = function appendToTagName(t) {
		this.current("StartTag", "EndTag").tagName += t;
	}, e.prototype.beginAttribute = function beginAttribute() {
		this._currentAttribute = [
			"",
			"",
			!1
		];
	}, e.prototype.appendToAttributeName = function appendToAttributeName(t) {
		this.currentAttribute()[0] += t;
	}, e.prototype.beginAttributeValue = function beginAttributeValue(t) {
		this.currentAttribute()[2] = t;
	}, e.prototype.appendToAttributeValue = function appendToAttributeValue(t) {
		this.currentAttribute()[1] += t;
	}, e.prototype.finishAttributeValue = function finishAttributeValue() {
		this.current("StartTag").attributes.push(this._currentAttribute);
	}, e.prototype.reportSyntaxError = function reportSyntaxError(t) {
...
function ye(e, t) {
	let r, s, n, { node: i, parent: a, parentKey: o } = t, c = (function c(h, f) {
...
function Vs(e, t, r, s) {
	let n, i, { node: a } = r, o = (function o(c, h) {
		return c[h];
	})(a, s);
	if (o) {
		if (t !== void 0) {
			let c = (function c(h, f) {
...
var Us = {
	mustache: function mustache(e, t = [], r = Ht([]), s = !1, n, i) {
		return d.mustache({
			path: rt(e),
			params: t,
			hash: r,
	...
	block: function block(e, t, r, s, n = null, i, a, o, c) {
		let h, f = null;
	...
	mustacheComment: function mustacheComment(e, t) {
	...
	elementModifier: function elementModifier(e, t, r, s) {
	...
	sexpr: function sexpr(e, t = [], r = Ht([]), s) {
		return d.sexpr({
			path: rt(e),
			params: t,
			hash: r,
	...
	literal: me,
	program: function program(e, t, r) {
	...
	path: rt,
	fullPath: function fullPath(e, t = [], r) {
		return d.path({
			head: e,
			tail: t,
			loc: v(r || null)
...
var fe = {
	...
	mustache({ path: e, params: t, hash: r, trusting: s, loc: n, strip: i = fe }) {
		return (function d({ path: a, params: o, hash: c, trusting: h, strip: f, loc: p }) {
			let g = {
	...
	path({ head: e, tail: t, loc: r }) {
		return (function d({ head: s, tail: n, loc: i }) {
			let a = {
				type: "PathExpression",
				head: s,
				tail: n,
	...
	literal({ type: e, value: t, loc: r }) {
		return (function d({ type: s, value: n, loc: i }) {
			let a = {
				type: s,
				value: n,
				loc: i
...
`);
	...
	ContentStatement(t) {
		(function ur(r, s) {
			let n = s.loc.start.line, i = s.loc.start.column, a = (function a(o, c) {
				if (c === "") return {
					lines: o.split(`
`).length - 1,
					columns: 0
