...
var method_at_default$1 = createMethodShim$1("at", function method_at_default$1() {
...
var stringReplaceAll$1 = String.prototype.replaceAll ?? function stringReplaceAll$1(pattern, replacement) {
	if (pattern.global) return this.replace(pattern, replacement);
	return this.split(pattern).join(replacement);
};
var method_replace_all_default$1 = createMethodShim$1("replaceAll", function method_replace_all_default$1() {
...
var __require = /* @__PURE__ */ ((x) => typeof require !== "undefined" ? require : typeof Proxy !== "undefined" ? new Proxy(x, { get: (a, b) => (typeof require !== "undefined" ? require : a)[b] }) : x)(function __require(x) {
...
var require_is_number = __commonJS({ "node_modules/is-number/index.js"(exports, module) {
	"use strict";
	module.exports = function exports(num) {
...
var require_queue = __commonJS({ "node_modules/fastq/queue.js"(exports, module) {
	...
	function queueAsPromised(context, worker, _concurrency) {
		...
		function push2(value) {
			var p = new Promise(function p(resolve3, reject) {
				pushCb(value, function p(err, result) {
					if (err) {
						reject(err);
						return;
					}
		...
		function unshift(value) {
			var p = new Promise(function p(resolve3, reject) {
				unshiftCb(value, function p(err, result) {
					if (err) {
						reject(err);
						return;
					}
...
var require_pseudomap = __commonJS({ "node_modules/pseudomap/pseudomap.js"(exports, module) {
	...
	PseudoMap.prototype.forEach = function forEach(fn, thisp) {
		thisp = thisp || this;
		Object.keys(this._data).forEach(function forEach(k) {
			if (k !== "size") fn.call(thisp, this._data[k].value, this._data[k].key);
		}, this);
	};
	PseudoMap.prototype.has = function has(k) {
		return !!find(this._data, k);
	};
	PseudoMap.prototype.get = function get(k) {
	...
	PseudoMap.prototype.clear = function clear() {
	...
	PseudoMap.prototype.values = PseudoMap.prototype.keys = PseudoMap.prototype.entries = function entries() {
...
var require_yallist = __commonJS({ "node_modules/yallist/yallist.js"(exports, module) {
	...
	Yallist.prototype.removeNode = function removeNode(node) {
	...
	Yallist.prototype.unshiftNode = function unshiftNode(node) {
	...
	Yallist.prototype.pushNode = function pushNode(node) {
	...
	Yallist.prototype.push = function push() {
	...
	Yallist.prototype.pop = function pop() {
		if (!this.tail) return;
	...
	Yallist.prototype.shift = function shift() {
		if (!this.head) return;
	...
	Yallist.prototype.forEach = function forEach(fn, thisp) {
		thisp = thisp || this;
	...
	Yallist.prototype.forEachReverse = function forEachReverse(fn, thisp) {
		thisp = thisp || this;
	...
	Yallist.prototype.mapReverse = function mapReverse(fn, thisp) {
		thisp = thisp || this;
		var res = new Yallist();
	...
	Yallist.prototype.reduce = function reduce(fn, initial) {
		var acc;
	...
	Yallist.prototype.reduceReverse = function reduceReverse(fn, initial) {
		var acc;
	...
	Yallist.prototype.toArray = function toArray() {
	...
	Yallist.prototype.toArrayReverse = function toArrayReverse() {
		var arr = new Array(this.length);
	...
	Yallist.prototype.slice = function slice(from, to) {
		to = to || this.length;
	...
	Yallist.prototype.sliceReverse = function sliceReverse(from, to) {
		to = to || this.length;
		if (to < 0) to += this.length;
		from = from || 0;
		if (from < 0) from += this.length;
	...
	Yallist.prototype.reverse = function reverse() {
		var head = this.head;
		var tail = this.tail;
...
var require_lru_cache = __commonJS({ "node_modules/editorconfig/node_modules/lru-cache/index.js"(exports, module) {
	...
	var makeSymbol;
	if (hasSymbol) makeSymbol = function makeSymbol(key2) {
		return Symbol(key2);
	};
	else makeSymbol = function makeSymbol(key2) {
		return "_" + key2;
	};
	...
	Object.defineProperty(LRUCache.prototype, "lengthCalculator", {
		set: function set(lC) {
			...
			if (lC !== this[LENGTH_CALCULATOR]) {
				this[LENGTH_CALCULATOR] = lC;
				this[LENGTH] = 0;
				this[LRU_LIST].forEach(function set(hit) {
	...
	LRUCache.prototype.keys = function keys() {
		return this[LRU_LIST].toArray().map(function keys(k) {
			return k.key;
		}, this);
	};
	LRUCache.prototype.values = function values() {
		return this[LRU_LIST].toArray().map(function values(k) {
			return k.value;
		}, this);
	};
	LRUCache.prototype.reset = function reset() {
		if (this[DISPOSE] && this[LRU_LIST] && this[LRU_LIST].length) this[LRU_LIST].forEach(function reset(hit) {
	...
	LRUCache.prototype.dump = function dump() {
		return this[LRU_LIST].map(function dump(hit) {
			if (!isStale(this, hit)) return {
				k: hit.key,
				v: hit.value,
				e: hit.now + (hit.maxAge || 0)
			};
		}, this).toArray().filter(function dump(h) {
			return h;
		});
	};
	...
	LRUCache.prototype.inspect = function inspect(n, opts) {
		var str = "LRUCache {";
		var extras = false;
		if (this[ALLOW_STALE]) {
			str += "\n  allowStale: true";
		...
		var didFirst = false;
		this[LRU_LIST].forEach(function inspect(item) {
			if (didFirst) str += ",\n  ";
			else {
				if (extras) str += ",\n";
				didFirst = true;
	...
	LRUCache.prototype.set = function set(key2, value, maxAge) {
	...
	LRUCache.prototype.has = function has(key2) {
	...
	LRUCache.prototype.peek = function peek(key2) {
	...
	LRUCache.prototype.prune = function prune() {
		var self = this;
		this[CACHE].forEach(function prune(value, key2) {
			get(self, key2, false);
		});
	};
...
var require_fnmatch = __commonJS({ "node_modules/editorconfig/src/lib/fnmatch.js"(exports, module) {
	...
	minimatch.defaults = function defaults(def) {
	...
	function make() {
		...
		set = this.globParts = set.map(function globParts(s) {
			return s.split(slashSplit);
		});
		if (options8.debug) console.error(this.pattern, set);
		set = set.map(function set(s, si, set2) {
			return s.map(this.parse, this);
		}, this);
		if (options8.debug) console.error(this.pattern, set);
		set = set.filter(function set(s) {
	...
	minimatch.braceExpand = function braceExpand(pattern, options8) {
	...
	function braceExpand(pattern, options8) {
		...
		set = set.map(function set(p) {
			return braceExpand(p, options8);
		});
		set = set.reduce(function set(l2, r) {
			return l2.concat(r);
		});
		if (addBraces) set = set.map(function set(s) {
			return "{" + s + "}";
		});
		var ret = [];
	...
	function parse7(pattern, isSub) {
		...
		while (pl = patternListStack.pop()) {
			...
			tail = tail.replace(/((?:\\{2})*)(\\?)\|/g, function tail(_, $1, $2) {
				if (!$2) $2 = "\\";
				return $1 + $1 + $2 + "|";
			});
	...
	minimatch.makeRe = function makeRe(pattern, options8) {
	...
	function makeRe() {
		...
		var re = set.map(function re(pattern) {
			return pattern.map(function re(p) {
	...
	minimatch.match = function match(list, pattern, options8) {
		var mm = new Minimatch(pattern, options8);
		list = list.filter(function list(f) {
			return mm.match(f);
		});
	...
	Minimatch.prototype.matchOne = function matchOne(file, pattern, partial) {
		var options8 = this.options;
...
var require_ini = __commonJS({ "node_modules/editorconfig/src/lib/ini.js"(exports) {
	"use strict";
	var __awaiter = exports && exports.__awaiter || function __awaiter(thisArg, _arguments, P, generator) {
		return new (P || (P = Promise))(function __awaiter(resolve3, reject) {
			...
			function step(result) {
				result.done ? resolve3(result.value) : new P(function __awaiter(resolve4) {
	...
	var __generator = exports && exports.__generator || function __generator(thisArg, body) {
		var _ = {
			label: 0,
			sent: function sent() {
				if (t[0] & 1) throw t[1];
	...
	var __importStar = exports && exports.__importStar || function __importStar(mod) {
...
var require_src = __commonJS({ "node_modules/editorconfig/src/index.js"(exports) {
	...
	var __importDefault = exports && exports.__importDefault || function __importDefault(mod) {
...
var require_js_tokens = __commonJS({ "node_modules/@babel/code-frame/node_modules/js-tokens/index.js"(exports, module) {
	...
	module.exports = function*  exports(input, { jsx = false } = {}) {
		...
		for (; componentPos < componentLen; componentPos++) {
			...
			if (!component.removed) {
				if (!component.added && this.useLongestToken) {
					...
					value = value.map(function value(value2, i) {
...
var tokenize2;
{
	const getTokenType = function getTokenType(token2) {
	...
	tokenize2 = function*  tokenize2(text) {
...
var captureLargerStackTrace = hideStackFrames(
	...
	* @returns {Error}
	*/
	function captureLargerStackTrace(error) {
...
var stringReplaceAll = String.prototype.replaceAll ?? function stringReplaceAll(pattern, replacement) {
	if (pattern.global) return this.replace(pattern, replacement);
	return this.split(pattern).join(replacement);
};
var method_replace_all_default = createMethodShim("replaceAll", function method_replace_all_default() {
...
var method_at_default = createMethodShim("at", function method_at_default() {
...
var arrayToReversed = Array.prototype.toReversed ?? function arrayToReversed() {
	return [...this].reverse();
};
var method_to_reversed_default = createMethodShim("toReversed", function method_to_reversed_default() {
...
var arrayFindLast = Array.prototype.findLast ?? function arrayFindLast(callback) {
	for (let index = this.length - 1; index >= 0; index--) {
		const element = this[index];
		if (callback(element, index, this)) return element;
	}
};
var method_find_last_default = createMethodShim("findLast", function method_find_last_default() {
...
var arrayFindLastIndex = Array.prototype.findLastIndex ?? function arrayFindLastIndex(callback) {
	for (let index = this.length - 1; index >= 0; index--) {
		const element = this[index];
...
var method_find_last_index_default = createMethodShim("findLastIndex", function method_find_last_index_default() {
