...
var require_polyfills = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function patch(fs) {
		...
		if (fs.chown && !fs.lchown) {
			fs.lchown = function lchown(path, uid, gid, cb) {
				if (cb) process.nextTick(cb);
			};
		...
		fs.readSync = typeof fs.readSync !== "function" ? fs.readSync : (function readSync(fs$readSync) {
		...
		function patchLchmod(fs) {
			fs.lchmod = function lchmod(path, mode, callback) {
				fs.open(path, constants.O_WRONLY | constants.O_SYMLINK, mode, function lchmod(err, fd) {
					if (err) {
						if (callback) callback(err);
						return;
					}
					fs.fchmod(fd, mode, function lchmod(err) {
						fs.close(fd, function lchmod(err2) {
		...
		function patchLutimes(fs) {
			if (constants.hasOwnProperty("O_SYMLINK") && fs.futimes) {
				fs.lutimes = function lutimes(path, at, mt, cb) {
					fs.open(path, constants.O_SYMLINK, function lutimes(er, fd) {
						if (er) {
							if (cb) cb(er);
							return;
						}
						fs.futimes(fd, at, mt, function lutimes(er) {
							fs.close(fd, function lutimes(er2) {
								if (cb) cb(er || er2);
							});
						});
					});
				};
				fs.lutimesSync = function lutimesSync(path, at, mt) {
				...
				fs.lutimes = function lutimes(_a, _b, _c, cb) {
					if (cb) process.nextTick(cb);
				};
				fs.lutimesSync = function lutimesSync() {};
			}
		}
		function chmodFix(orig) {
			if (!orig) return orig;
			return function require_polyfills(target, mode, cb) {
				return orig.call(fs, target, mode, function require_polyfills(er) {
		...
		function chmodFixSync(orig) {
			if (!orig) return orig;
			return function require_polyfills(target, mode) {
				try {
		...
		function chownFix(orig) {
			if (!orig) return orig;
			return function require_polyfills(target, uid, gid, cb) {
				return orig.call(fs, target, uid, gid, function require_polyfills(er) {
					if (chownErOk(er)) er = null;
					if (cb) cb.apply(this, arguments);
				});
			};
		...
		function chownFixSync(orig) {
			if (!orig) return orig;
			return function require_polyfills(target, uid, gid) {
				try {
		...
		function statFix(orig) {
			if (!orig) return orig;
			return function require_polyfills(target, options, cb) {
				if (typeof options === "function") {
					cb = options;
					options = null;
				}
		...
		function statFixSync(orig) {
			if (!orig) return orig;
			return function require_polyfills(target, options) {
...
var require_legacy_streams = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function legacy(fs) {
		...
		function ReadStream(path, options) {
			...
			if (this.fd !== null) {
				process.nextTick(function require_legacy_streams() {
					self._read();
				});
				return;
			}
			fs.open(this.path, this.flags, this.mode, function require_legacy_streams(err, fd) {
				if (err) {
					self.emit("error", err);
					self.readable = false;
					return;
...
var require_clone = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function clone(obj) {
		...
		Object.getOwnPropertyNames(obj).forEach(function require_clone(key) {
...
var require_graceful_fs = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	function publishQueue(context, queue) {
		Object.defineProperty(context, gracefulQueue, { get: function get() {
			return queue;
		} });
	}
	var debug = noop;
	...
	if (!fs$1[gracefulQueue]) {
		...
		fs$1.close = (function(fs$close) {
			function close(fd, cb) {
				return fs$close.call(fs$1, fd, function close(err) {
					if (!err) resetQueue();
		...
		if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || "")) process.on("exit", function require_graceful_fs() {
	...
	function patch(fs) {
		...
		function readFile(path, options, cb) {
			...
			function go$readFile(path, options, cb, startTime) {
				return fs$readFile(path, options, function require_graceful_fs(err) {
		...
		function writeFile(path, data, options, cb) {
			...
			function go$writeFile(path, data, options, cb, startTime) {
				return fs$writeFile(path, data, options, function require_graceful_fs(err) {
					if (err && (err.code === "EMFILE" || err.code === "ENFILE")) enqueue([
						go$writeFile,
						[
							path,
		...
		function appendFile(path, data, options, cb) {
			...
			function go$appendFile(path, data, options, cb, startTime) {
				return fs$appendFile(path, data, options, function require_graceful_fs(err) {
					if (err && (err.code === "EMFILE" || err.code === "ENFILE")) enqueue([
						go$appendFile,
						[
							path,
		...
		function copyFile(src, dest, flags, cb) {
			...
			function go$copyFile(src, dest, flags, cb, startTime) {
				return fs$copyFile(src, dest, flags, function require_graceful_fs(err) {
					if (err && (err.code === "EMFILE" || err.code === "ENFILE")) enqueue([
						go$copyFile,
						[
							src,
		...
		function readdir(path, options, cb) {
			...
			function fs$readdirCallback(path, options, cb, startTime) {
				return function require_graceful_fs(err, files) {
					if (err && (err.code === "EMFILE" || err.code === "ENFILE")) enqueue([
						go$readdir,
						[
							path,
		...
		function ReadStream$open() {
			var that = this;
			open(that.path, that.flags, that.mode, function require_graceful_fs(err, fd) {
				if (err) {
		...
		function open(path, flags, mode, cb) {
			...
			function go$open(path, flags, mode, cb, startTime) {
				return fs$open(path, flags, mode, function require_graceful_fs(err, fd) {
					if (err && (err.code === "EMFILE" || err.code === "ENFILE")) enqueue([
						go$open,
						[
							path,
