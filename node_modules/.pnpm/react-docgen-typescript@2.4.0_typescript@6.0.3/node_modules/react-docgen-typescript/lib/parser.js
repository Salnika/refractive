...
var __spreadArray = (this && this.__spreadArray) || function  __spreadArray(to, from) {
...
function withCustomConfig(tsconfigPath, parserOpts) {
    ...
    var _a = ts.readConfigFile(tsconfigPath, function  _a(filename) {
...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.getTypeSymbol = function  getTypeSymbol(exp) {
    ...
    Parser.prototype.isPlainObjectType = function  isPlainObjectType(exp) {
     ...
     * @param exp symbol
     */
    Parser.prototype.getComponentExports = function  getComponentExports(exp) {
        var targetSymbol = exp;
    ...
    Parser.prototype.getComponentFromExpression = function  getComponentFromExpression(exp) {
        var declaration = exp.valueDeclaration || exp.declarations[0];
    ...
    Parser.prototype.getComponentInfo = function  getComponentInfo(exp, source, componentNameResolver, customComponentTypes) {
        if (componentNameResolver === void 0) { componentNameResolver = function  componentNameResolver() { return undefined; }; }
    ...
    Parser.prototype.extractPropsFromTypeIfStatelessComponent = function  extractPropsFromTypeIfStatelessComponent(type) {
    ...
    Parser.prototype.extractPropsFromTypeIfStatefulComponent = function  extractPropsFromTypeIfStatefulComponent(type) {
    ...
    Parser.prototype.extractMembersFromType = function  extractMembersFromType(type) {
        var _this = this;
        var methodSymbols = [];
        /**
         ...
         */
        type.getProperties().forEach(function  extractMembersFromType(property) {
        ...
        if (type.symbol && type.symbol.members) {
            type.symbol.members.forEach(function  extractMembersFromType(member) {
                methodSymbols.push(member);
            });
        }
        return methodSymbols;
    };
    Parser.prototype.getMethodsInfo = function  getMethodsInfo(type) {
        var _this = this;
        var members = this.extractMembersFromType(type);
        var methods = [];
        members.forEach(function  getMethodsInfo(member) {
    ...
    Parser.prototype.getModifiers = function  getModifiers(member) {
        var modifiers = [];
    ...
    Parser.prototype.getParameterInfo = function  getParameterInfo(callSignature) {
        var _this = this;
    ...
    Parser.prototype.getCallSignature = function  getCallSignature(symbol) {
    ...
    Parser.prototype.isTaggedPublic = function  isTaggedPublic(symbol) {
        var jsDocTags = symbol.getJsDocTags();
        return Boolean(jsDocTags.find(function  isTaggedPublic(tag) { return tag.name === 'public'; }));
    };
    Parser.prototype.getReturnDescription = function  getReturnDescription(symbol) {
        var tags = symbol.getJsDocTags();
        var returnTag = tags.find(function  returnTag(tag) { return tag.name === 'returns'; });
        if (!returnTag || !Array.isArray(returnTag.text)) {
            return;
        }
        return returnTag.text;
    };
    Parser.prototype.getValuesFromUnionType = function  getValuesFromUnionType(type) {
    ...
    Parser.prototype.getInfoFromUnionType = function  getInfoFromUnionType(type) {
        var commentInfo = {};
        if (type.getSymbol()) {
    ...
    Parser.prototype.getDocgenType = function  getDocgenType(propType, isRequired) {
        var _this = this;
        ...
        if (propType.isUnion()) {
            if (this.shouldExtractValuesFromUnion ||
                (this.shouldExtractLiteralValuesFromEnum &&
                    propType.types.every(function  getDocgenType(type) {
                ...
                var value = propType.types.map(function  value(type) { return _this.getInfoFromUnionType(type); });
                if (this.shouldRemoveUndefinedFromOptional && !isRequired) {
                    value = value.filter(function  value(option) { return option.value != 'undefined'; });
                }
                if (this.shouldSortUnions) {
                    value.sort(function  getDocgenType(a, b) {
    ...
    Parser.prototype.getPropsInfo = function  getPropsInfo(propsObj, defaultProps) {
        var _this = this;
        ...
        if (propsType.isUnionOrIntersection()) {
            ...
            if (!propertiesOfProps.length) {
                ...
                // @ts-ignore
                function  subTypes(all, t) { return __spreadArray(__spreadArray([], all), (t.types || [])); }, []));
                propertiesOfProps = __spreadArray(__spreadArray([], subTypes), baseProps);
            }
        }
        var result = {};
        propertiesOfProps.forEach(function  getPropsInfo(prop) {
            ...
            if (_this.propertiesOfPropsCache.has(cacheKey)) {
                ...
                var baseProp = baseProps.find(function  baseProp(p) { return p.getName() === propName; });
                var required = !isOptional(prop) &&
                    !hasCodeBasedDefault &&
                    // If in a intersection or union check original declaration for "?"
                    // @ts-ignore
                    declarations.every(function  required(d) { return !d.questionToken; }) &&
    ...
    Parser.prototype.findDocComment = function  findDocComment(symbol) {
        var _this = this;
        ...
        var commentsOnRootSymbols = rootSymbols
            .filter(function  commentsOnRootSymbols(x) { return x !== symbol; })
            .map(function  commentsOnRootSymbols(x) { return _this.getFullJsDocComment(x); })
            .filter(function  commentsOnRootSymbols(x) { return !!x.fullComment || !!comment.tags.default; });
     ...
     * text and JsDoc tags.
     */
    Parser.prototype.getFullJsDocComment = function  getFullJsDocComment(symbol) {
        ...
        var tagComments = [];
        var tagMap = {};
        tags.forEach(function  getFullJsDocComment(tag) {
    ...
    Parser.prototype.getFunctionStatement = function  getFunctionStatement(statement) {
    ...
    Parser.prototype.extractDefaultPropsFromComponent = function  extractDefaultPropsFromComponent(symbol, source) {
        var _this = this;
        var possibleStatements = __spreadArray(__spreadArray([], source.statements
            // ensure that name property is available
            .filter(function  possibleStatements(stmt) { return !!stmt.name; })
            .filter(function  possibleStatements(stmt) {
            return _this.checker.getSymbolAtLocation(stmt.name) === symbol;
        })), source.statements.filter(function  possibleStatements(stmt) { return ts.isExpressionStatement(stmt) || ts.isVariableStatement(stmt); }));
        return possibleStatements.reduce(function  extractDefaultPropsFromComponent(res, statement) {
            if (statementIsClassDeclaration(statement) && statement.members.length) {
                ...
                var propMap_1 = {};
                statement.getChildren().forEach(function  extractDefaultPropsFromComponent(child) {
    ...
    Parser.prototype.getLiteralValueFromImportSpecifier = function  getLiteralValueFromImportSpecifier(property) {
    ...
    Parser.prototype.getLiteralValueFromPropertyAssignment = function  getLiteralValueFromPropertyAssignment(property) {
    ...
    Parser.prototype.getPropMap = function  getPropMap(properties) {
        var _this = this;
...
function getTextValueOfClassMember(classDeclaration, memberName) {
    ...
    var textValue = (classDeclarationMembers &&
        classDeclarationMembers
            .filter(function  textValue(member) { return ts.isPropertyDeclaration(member); })
...
function getTextValueOfFunctionProperty(exp, source, propertyName) {
    var identifierStatements = source.statements
        .filter(function  identifierStatements(statement) { return ts.isExpressionStatement(statement); })
        .filter(function  identifierStatements(statement) {
        var _a, _b, _c, _d;
        var expr = statement
            .expression;
        var locals = Array.from(source.locals);
        var hasOneLocalExport = locals.filter(function  hasOneLocalExport(local) { return !!local[1].exports; }).length === 1;
    ...
    if (identifierStatements.length > 0) {
        var locatedStatement = identifierStatements.find(function  locatedStatement(statement) { return statement[0] === exp.escapedName; });
        if (locatedStatement) {
...
function parseWithProgramProvider(filePathOrPaths, compilerOptions, parserOpts, programProvider) {
    ...
    return filePaths
        ...
        var filteredComponentDocs = componentDocsNoDuplicates.filter(function  filteredComponentDocs(comp, index, comps) {
            return comps
                .slice(index + 1)
                .every(function  filteredComponentDocs(innerComp) { return innerComp.displayName !== comp.displayName; });
        });
