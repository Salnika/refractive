...
var Lines = /** @class */ (function () {
    ...
    Lines.prototype.toString = function  toString(options) {
        return this.sliceString(this.firstPos(), this.lastPos(), options);
    };
    Lines.prototype.getSourceMap = function  getSourceMap(sourceMapName, sourceRoot) {
        if (!sourceMapName) {
        ...
        targetLines.mappings.forEach(function  getSourceMap(mapping) {
    ...
    Lines.prototype.bootstrapCharAt = function  bootstrapCharAt(pos) {
    ...
    Lines.prototype.charAt = function  charAt(pos) {
        (0, tiny_invariant_1.default)(typeof pos === "object");
        (0, tiny_invariant_1.default)(typeof pos.line === "number");
        (0, tiny_invariant_1.default)(typeof pos.column === "number");
    ...
    Lines.prototype.stripMargin = function  stripMargin(width, skipFirstLine) {
        if (width === 0)
            return this;
        (0, tiny_invariant_1.default)(width > 0, "negative margin: " + width);
        if (skipFirstLine && this.length === 1)
            return this;
        var lines = new Lines(this.infos.map(function  lines(info, i) {
        ...
        if (this.mappings.length > 0) {
            ...
            this.mappings.forEach(function  stripMargin(mapping) {
                newMappings_1.push(mapping.indent(width, skipFirstLine, true));
            });
        }
        return lines;
    };
    Lines.prototype.indent = function  indent(by) {
        if (by === 0) {
            return this;
        }
        var lines = new Lines(this.infos.map(function  lines(info) {
        ...
        if (this.mappings.length > 0) {
            ...
            this.mappings.forEach(function  indent(mapping) {
                newMappings_2.push(mapping.indent(by));
            });
        }
        return lines;
    };
    Lines.prototype.indentTail = function  indentTail(by) {
        if (by === 0) {
            return this;
        }
        if (this.length < 2) {
        ...
        if (this.mappings.length > 0) {
            ...
            this.mappings.forEach(function  indentTail(mapping) {
                newMappings_3.push(mapping.indent(by, true));
            });
        }
        return lines;
    };
    Lines.prototype.lockIndentTail = function  lockIndentTail() {
        if (this.length < 2) {
            return this;
        }
        return new Lines(this.infos.map(function  lockIndentTail(info, i) { return (tslib_1.__assign(tslib_1.__assign({}, info), { locked: i > 0 })); }));
    };
    Lines.prototype.getIndentAt = function  getIndentAt(line) {
        (0, tiny_invariant_1.default)(line >= 1, "no line " + line + " (line numbers start from 1)");
        return Math.max(this.infos[line - 1].indent, 0);
    };
    Lines.prototype.guessTabWidth = function  guessTabWidth() {
    ...
    Lines.prototype.startsWithComment = function  startsWithComment() {
    ...
    Lines.prototype.isPrecededOnlyByWhitespace = function  isPrecededOnlyByWhitespace(pos) {
    ...
    Lines.prototype.getLineLength = function  getLineLength(line) {
        var info = this.infos[line - 1];
        return this.getIndentAt(line) + info.sliceEnd - info.sliceStart;
    };
    Lines.prototype.nextPos = function  nextPos(pos, skipSpaces) {
    ...
    Lines.prototype.prevPos = function  prevPos(pos, skipSpaces) {
        if (skipSpaces === void 0) { skipSpaces = false; }
    ...
    Lines.prototype.firstPos = function  firstPos() {
        // Trivial, but provided for completeness.
        return { line: 1, column: 0 };
    };
    Lines.prototype.lastPos = function  lastPos() {
        return {
            line: this.length,
            column: this.getLineLength(this.length),
        };
    };
    Lines.prototype.skipSpaces = function  skipSpaces(pos, backward, modifyInPlace) {
    ...
    Lines.prototype.trimLeft = function  trimLeft() {
        var pos = this.skipSpaces(this.firstPos(), false, true);
        return pos ? this.slice(pos) : emptyLines;
    };
    Lines.prototype.trimRight = function  trimRight() {
        var pos = this.skipSpaces(this.lastPos(), true, true);
        return pos ? this.slice(this.firstPos(), pos) : emptyLines;
    };
    Lines.prototype.trim = function  trim() {
    ...
    Lines.prototype.eachPos = function  eachPos(callback, startPos, skipSpaces) {
    ...
    Lines.prototype.bootstrapSlice = function  bootstrapSlice(start, end) {
    ...
    Lines.prototype.slice = function  slice(start, end) {
        if (!end) {
            if (!start) {
        ...
        if (this.mappings.length > 0) {
            ...
            this.mappings.forEach(function  slice(mapping) {
    ...
    Lines.prototype.bootstrapSliceString = function  bootstrapSliceString(start, end, options) {
        return this.slice(start, end).toString(options);
    };
    Lines.prototype.sliceString = function  sliceString(start, end, options) {
    ...
    Lines.prototype.join = function  join(elements) {
        var separator = this;
        var infos = [];
        var mappings = [];
        var prevInfo;
        function appendLines(linesOrNull) {
            ...
            if (prevInfo) {
                ...
                if (linesOrNull.mappings.length > 0) {
                    linesOrNull.mappings.forEach(function  join(mapping) {
            ...
            linesOrNull.infos.forEach(function  join(info, i) {
                if (!prevInfo || i > 0) {
        ...
        elements
            .map(function  join(elem) {
            var lines = fromString(elem);
            if (lines.isEmpty())
                return null;
            return lines;
        })
            .forEach(function  join(linesOrNull, i) {
    ...
    Lines.prototype.concat = function  concat() {
        var args = [];
...
function fromString(string, options) {
    ...
    var lines = new Lines(string.split(lineTerminatorSeqExp).map(function  lines(line) {
