...
var require_warning = __commonJS({
  "../../node_modules/warning/warning.js"(exports, module) {
    "use strict";
    var __DEV__ = process.env.NODE_ENV !== "production", warning2 = function warning2() {
    };
    __DEV__ && (printWarning = function printWarning(format, args) {
      var len = arguments.length;
      args = new Array(len > 1 ? len - 1 : 0);
      for (var key = 1; key < len; key++)
        args[key - 1] = arguments[key];
      var argIndex = 0, message = "Warning: " + format.replace(/%s/g, function message() {
    ...
    }, warning2 = function warning2(condition, format, args) {
      var len = arguments.length;
...
var fromEntries = function fromEntries(entries) {
  return entries.reduce(function fromEntries(acc, _ref) {
...
var top = "top", bottom = "bottom", right = "right", left = "left", auto = "auto", basePlacements = [top, bottom, right, left], start = "start", end = "end", clippingParents = "clippingParents", viewport = "viewport", popper = "popper", reference = "reference", variationPlacements = basePlacements.reduce(function variationPlacements(acc, placement) {
  return acc.concat([placement + "-" + start, placement + "-" + end]);
}, []), placements = [].concat(basePlacements, [auto]).reduce(function placements(acc, placement) {
...
function effect(_ref2) {
  ...
  return Object.assign(state.elements.popper.style, initialStyles.popper), state.styles = initialStyles, state.elements.arrow && Object.assign(state.elements.arrow.style, initialStyles.arrow), function() {
    Object.keys(state.elements).forEach(function(name) {
      var element = state.elements[name], attributes = state.attributes[name] || {}, styleProperties = Object.keys(state.styles.hasOwnProperty(name) ? state.styles[name] : initialStyles[name]), style = styleProperties.reduce(function style(style2, property) {
...
function getClippingRect(element, boundary, rootBoundary, strategy) {
  var mainClippingParents = boundary === "clippingParents" ? getClippingParents(element) : [].concat(boundary), clippingParents2 = [].concat(mainClippingParents, [rootBoundary]), firstClippingParent = clippingParents2[0], clippingRect = clippingParents2.reduce(function clippingRect(accRect, clippingParent) {
...
function flip(_ref) {
  ...
  if (!state.modifiersData[name]._skip) {
    for (var _options$mainAxis = options.mainAxis, checkMainAxis = _options$mainAxis === void 0 ? !0 : _options$mainAxis, _options$altAxis = options.altAxis, checkAltAxis = _options$altAxis === void 0 ? !0 : _options$altAxis, specifiedFallbackPlacements = options.fallbackPlacements, padding = options.padding, boundary = options.boundary, rootBoundary = options.rootBoundary, altBoundary = options.altBoundary, _options$flipVariatio = options.flipVariations, flipVariations = _options$flipVariatio === void 0 ? !0 : _options$flipVariatio, allowedAutoPlacements = options.allowedAutoPlacements, preferredPlacement = state.options.placement, basePlacement = getBasePlacement(preferredPlacement), isBasePlacement = basePlacement === preferredPlacement, fallbackPlacements = specifiedFallbackPlacements || (isBasePlacement || !flipVariations ? [getOppositePlacement(preferredPlacement)] : getExpandedFallbackPlacements(preferredPlacement)), placements2 = [preferredPlacement].concat(fallbackPlacements).reduce(function placements2(acc, placement2) {
    ...
    if (makeFallbackChecks)
      for (var numberOfChecks = flipVariations ? 3 : 1, _loop = function _loop(_i2) {
        var fittingPlacement = placements2.find(function fittingPlacement(placement2) {
          var checks2 = checksMap.get(placement2);
          if (checks2)
            return checks2.slice(0, _i2).every(function fittingPlacement(check) {
              return check;
            });
        });
        if (fittingPlacement)
...
function offset(_ref2) {
  var state = _ref2.state, options = _ref2.options, name = _ref2.name, _options$offset = options.offset, offset2 = _options$offset === void 0 ? [0, 0] : _options$offset, data = placements.reduce(function data(acc, placement) {
...
function mergeByName(modifiers) {
  var merged = modifiers.reduce(function merged(merged2, current) {
  ...
  return function(reference2, popper2, options) {
    ...
    var state = {
      ...
      setOptions: function setOptions(setOptionsAction) {
        ...
        return state.orderedModifiers = orderedModifiers.filter(function orderedModifiers(m) {
          return m.enabled;
      ...
      forceUpdate: function forceUpdate() {
        if (!isDestroyed) {
          ...
          if (areValidElements(reference3, popper3)) {
            ...
            }, state.reset = !1, state.placement = state.options.placement, state.orderedModifiers.forEach(function forceUpdate(modifier) {
...
var EMPTY_MODIFIERS = [], usePopper = function usePopper(referenceElement, popperElement, options) {
  options === void 0 && (options = {});
  var prevOptions = React3.useRef(null), optionsWithDefaults = {
    ...
    attributes: {}
  }), state = _React$useState[0], setState = _React$useState[1], updateStateModifier = React3.useMemo(function updateStateModifier() {
    return {
      name: "updateState",
      enabled: !0,
      phase: "write",
      fn: function fn(_ref) {
        ...
        ReactDOM.flushSync(function fn() {
          setState({
            styles: fromEntries(elements.map(function styles(element) {
              return [element, state2.styles[element] || {}];
            })),
            attributes: fromEntries(elements.map(function attributes(element) {
  ...
  }, []), popperOptions = React3.useMemo(function popperOptions() {
    var newOptions = {
  ...
  return useIsomorphicLayoutEffect(function usePopper() {
    popperInstanceRef.current && popperInstanceRef.current.setOptions(popperOptions);
  }, [popperOptions]), useIsomorphicLayoutEffect(function usePopper() {
    if (!(referenceElement == null || popperElement == null)) {
      ...
      return popperInstanceRef.current = popperInstance, function usePopper() {
...
function useControlledState(_ref) {
  ...
  var _React$useState = React6.useState(initial), state = _React$useState[0], setState = _React$useState[1], getLatest = useGetLatest(state), set = React6.useCallback(function set(updater) {
...
function usePopperTooltip(config, popperOptions) {
  ...
  var finalConfig = Object.keys(defaultConfig).reduce(function finalConfig(config2, key) {
    var _extends2;
  ...
  var _usePopper = usePopper(finalConfig.followCursor ? virtualElement : triggerRef, tooltipRef, finalPopperOptions), styles = _usePopper.styles, attributes = _usePopper.attributes, popperProps = _objectWithoutPropertiesLoose(_usePopper, _excluded), update = popperProps.update, getLatest = useGetLatest({
    ...
    function isTriggeredBy(trigger) {
  ...
  ), hideTooltip = React6.useCallback(function hideTooltip() {
    clearTimeout(timer.current), timer.current = window.setTimeout(function current() {
      return setVisible(!1);
    }, finalConfig.delayHide);
  }, [finalConfig.delayHide, setVisible]), showTooltip = React6.useCallback(function showTooltip() {
    clearTimeout(timer.current), timer.current = window.setTimeout(function current() {
      return setVisible(!0);
    }, finalConfig.delayShow);
  }, [finalConfig.delayShow, setVisible]), toggleTooltip = React6.useCallback(function toggleTooltip() {
  ...
  }, getArrowProps = function getArrowProps(args) {
