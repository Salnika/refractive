...
var require_picocolors_browser = __commonJS({
  "../../node_modules/picocolors/picocolors.browser.js"(exports, module) {
    var x = String, create2 = function create2() {
      ...
      var _ = {
        ...
        storeAPI: {
          ...
          keys: function keys(fillList) {
            return this.each(function keys(k, v, list) {
              list.push(k);
            }, fillList || []);
          },
          ...
          getAll: function getAll(fillObj) {
            return this.each(function getAll(k, v, all) {
              all[k] = v;
            }, fillObj || {});
          },
          ...
          clear: function clear() {
            return this._ns ? this.each(function clear(k) {
...
var init4 = ({ store: store2, fullAPI, provider }) => {
  ...
  return provider.channel?.on(
    GLOBALS_UPDATED,
    function init4({ globals, storyGlobals, userGlobals }) {
    ...
    SET_GLOBALS,
    function init4({ globals, globalTypes }) {
...
var init11 = ({
  ...
  provider.channel?.on(
  ...
  ), provider.channel?.on(CURRENT_STORY_WAS_SET, function init11() {
    let { ref } = getEventMetadata(this, fullAPI);
    api.setPreviewInitialized(ref);
  }), provider.channel?.on(STORY_CHANGED, function init11() {
    let { sourceType } = getEventMetadata(this, fullAPI);
    if (sourceType === "local") {
  ...
  ), provider.channel?.on(SET_INDEX, function init11(index) {
    let { ref } = getEventMetadata(this, fullAPI);
    if (ref)
  ...
  }), provider.channel?.on(SET_STORIES, function init11(data) {
    ...
    STORY_ARGS_UPDATED,
    function init11({ storyId, args }) {
      let { ref } = getEventMetadata(this, fullAPI);
      api.updateStory(storyId, { args }, ref);
    }
  ), provider.channel?.on(CONFIG_ERROR, function init11(err) {
    let { ref } = getEventMetadata(this, fullAPI);
    api.setPreviewInitialized(ref);
  }), provider.channel?.on(STORY_MISSING, function init11(err) {
    let { ref } = getEventMetadata(this, fullAPI);
    api.setPreviewInitialized(ref);
...
var store_setup_default = (_) => {
  _.fn("set", function store_setup_default(key, data) {
    return _.set(
  ...
  }), _.fn("get", function store_setup_default(key, alt) {
