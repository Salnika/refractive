...
function mergeEditorConfigs(existing, incoming, fileName) {
	if (fileName === "extensions.json") {
		const existingRecs = Array.isArray(existing["recommendations"]) ? existing["recommendations"] : [];
...
function deepMerge(target, source) {
	const result = { ...target };
	for (const [key, value] of Object.entries(source)) if (!(key in result)) result[key] = value;
	else if (typeof result[key] === "object" && result[key] !== null && !Array.isArray(result[key]) && typeof value === "object" && value !== null && !Array.isArray(value)) result[key] = deepMerge(result[key], value);
	return result;
}
